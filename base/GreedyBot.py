...
#
#    try:
#        if True or len(sys.argv) > 1: # Run LocalEngine version
    engine = LocalEngine()
    engine.AddBot(GreedyBot(engine.GetWorld()))
    engine.AddBot(GreedyBot(engine.GetWorld()))
    engine.Run('--run -t 1000 --map_file=maps/2player/funhouse.map --food=random --step-through=0 --viewradius2=999'.split())
